...
{
    // std::filesystem::path currentPath = std::filesystem::current_path() / "Content" / "assets.of";

    // of::Reader reader(currentPath);
    // of::Document document;
    // document.Decompile(reader.Buffer());

    // of::Object boar = document["Boar"];
    // of::Variable path = boar["path"];
    // of::Variable size = boar["size"];

    // auto strPath = path.AsValue<std::string>();
    // auto strSize = size.AsArray<double>();
